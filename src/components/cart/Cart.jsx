...
function Cart({ items, removeFromCart, vaciarCarrito }) {
  ...
  const submitOrder = (comprador) => {
    ...
    addDoc(ordersCollection, order).then(({ id }) => {
      Swal.fire("Orden enviada :)", `Su orden es la ${id}`, "success");
      navigate("/");
      vaciarCarrito();
    });
  };
  ...
  return (
    <div>
      ...
      <div>
        <a
          class="waves-effect waves-light btn-large blue lighten-2"
          onClick={finalizarCompra}
        >
          <i class="material-icons right">sentiment_very_satisfied</i>Finalizar
          compra
        </a>
        <a
        ...
        >
          <i class="material-icons right">remove_shopping_cart</i>Vaciar Carrito
        </a>
      </div>
    </div>
  );
